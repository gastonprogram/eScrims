...
public class BuscarScrimView {
    ...
    public void mostrarDetalleScrim(Scrim scrim) {
        ...
        if (!scrim.getPostulacionesAceptadas().isEmpty()) {
            System.out.println("\nPostulados:");
            for (dominio.modelo.Postulacion postulacion : scrim.getPostulacionesAceptadas()) {
    ...
    /**
     * Construye los filtros de búsqueda solicitando todos los criterios al usuario.
     */
    public aplicacion.builders.FiltrosScrim solicitarFiltros() {
        mostrarTitulo();

        aplicacion.builders.FiltrosScrim.Builder builder = new aplicacion.builders.FiltrosScrim.Builder();
    ...
    /**
     * Confirma si se desea eliminar un scrim.
     */
    public boolean confirmarEliminacion(Scrim scrim) {
        System.out.println("\n¿Está seguro que desea eliminar este scrim?");
        mostrarDetalleScrim(scrim);
    ...
    /**
     * Muestra una lista de scrims en formato resumido.
     */
    public void mostrarListaScrims(List<Scrim> scrims) {
        ...
        for (int i = 0; i < scrims.size(); i++) {
            Scrim scrim = scrims.get(i);
            System.out.println((i + 1) + ". " + scrim.getId().substring(0, 8) + "... - "
                    + scrim.getJuego().getNombre() + " - "
                    + scrim.getFormato().getFormatName() + " - "
                    + scrim.getFechaHora().format(FORMATO_FECHA) + " - "
                    + "Estado: " + scrim.getEstado());
        }
    }
